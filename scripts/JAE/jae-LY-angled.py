...
JAE LY series horizontal entry dual row
"""
...
#JAE part numbers
code = "LY20-{n:02}P-2LT"

#part description format strings
part_name = "JAE_{pn}_2x{n:02}x{p:.2f}mm_Angled"
part_description = "JAE LY series connector, dual row side entry, through hole, Datasheet:{ds}".format(ds=datasheet)
...
if __name__ == '__main__':
    ...
    for pins in pincount:
        ...
        footprint.append(Text(type='value', text=fp_name, at=[A/2,4], layer='F.Fab'))
        ...
        if pins < 5:
            ...
            outline = [
                ...
                {'x': x1 - o,'y': y1 - o},
                {'x': A/2 - C/2,'y': y1 - o},
                {'x': A/2 - C/2,'y': y1 - o + 2.8},
                {'x': A/2,'y': y1 - o + 2.8},
                
            ]
        
        footprint.append(PolygoneLine(polygone=outline))
        footprint.append(PolygoneLine(polygone=outline,x_mirror=A/2))
        
        #draw each pair of pins         
        if pins >= 5:
            w = 0.25
            for p in range(pins):
                if p < 2 or p > (pins - 3): continue
                x = p * pitch
