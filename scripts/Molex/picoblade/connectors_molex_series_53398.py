...
class footprintParams():
    tags = "connector PicoBlade header Molex"
    ...
    pin_pad_size = [0.8,1.3]
    pin_pad_center_y = -2.75
...
def generate_footprint(num_pins, lib_name = "Connectors_Molex", ref_on_ffab=False):
    ...
    y_silk_mount_body_top = footprintParams.y_mount_body_top - footprintParams.silk_to_body_offset
    ...
    if y_silk_mount_body_top <= y_silk_mount_pad_top:
        poly_silk_left_top.append({'x':-dx_main_silk, 'y':y_silk_mount_body_top})
        poly_silk_left_top.append({'x':-dx_mount_silk, 'y':y_silk_mount_body_top})
    else:
        poly_silk_left_top.append({'x':-dx_main_silk, 'y':y_silk_mount_pad_top})

    if y_silk_mount_body_top < y_silk_mount_pad_top:
        poly_silk_left_top.append({'x':-dx_mount_silk, 'y':y_silk_mount_pad_top})
    ...
    if y_silk_mount_body_top <= y_silk_mount_pad_top:
        poly_silk_right_top.append({'x':dx_main_silk, 'y':y_silk_mount_body_top})
        poly_silk_right_top.append({'x':dx_mount_silk, 'y':y_silk_mount_body_top})
    else:
        poly_silk_right_top.append({'x':dx_main_silk, 'y':y_silk_mount_pad_top})

    if y_silk_mount_body_top < y_silk_mount_pad_top:
        poly_silk_right_top.append({'x':dx_mount_silk, 'y':y_silk_mount_pad_top})
    ...
    dx_bottom_silk = footprintParams.mount_pad_center_x_to_pin + outher_pin_to_0 - footprintParams.mount_pad_size[0]/2 - footprintParams.silk_to_pad_min_distance

    poly_silk_bottom.append({'x':-dx_bottom_silk, 'y':y_silk_bottom})
    poly_silk_bottom.append({'x':dx_bottom_silk, 'y':y_silk_bottom})
    ...
    crtyd_bottom = footprintParams.mount_pad_center_y + footprintParams.mount_pad_size[1]/2.0 + footprintParams.courtyard_distance
